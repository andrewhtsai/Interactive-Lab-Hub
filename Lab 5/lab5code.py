...
while(1):
    ...
    red_mask = cv2.inRange(frame_color, red_lower, red_upper)

    blue_lower = np.array([94, 120, 100], np.uint8)
    blue_upper = np.array([120, 255, 255], np.uint8)
    blue_mask = cv2.inRange(frame_color, blue_lower, blue_upper)    
    ...
    if (number<4):
        print("red points evacuated")
        closeEverything()
        print("exported to flow.png")
        os.system("echo 'Observed Event!' | mail -s 'From _Another Set of Eyes_' 'sn438@cornell.edu' -A flow.png")
    ...
    if args.window=='window':
        cv2.imshow('optical flow (press q to quit.)',img)
